...
from .copy_generator import collapse_copy_scores

class CopySequenceGenerator(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
    ...
    def generate(
        ...
        **kwargs
        ...
        oov_len = src_map_orig.size()[2]
        beam_size = self.beam_size
        batch_offset=torch.arange(bsz, dtype=torch.long)
        ...
        src_map = src_map_orig.index_select(0, new_order)
        ...
        def collapse(scores, decoder_in):
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs = collapse(lprobs, encoder_outs[0]['encoder_out'])
...
class EnsembleModel(torch.nn.Module):
    ...
    def forward_decoder(self, tokens, encoder_outs, temperature=1., sample=None):
        ...
        raise Exception('Ensemble models not supported')
    ...
    def _decode_one(
        ...
        decoder_out[0] = list(decoder_out[0])
        decoder_out[0][0] = decoder_out[0][0][:, -1:, :]
        if temperature != 1.:
            decoder_out[0][0].div_(temperature)
        decoder_out[0][1] = decoder_out[0][1][:, -1:, :]
        if temperature != 1.:
            decoder_out[0][1].div_(temperature)

        attn = decoder_out[1]
        if type(attn) is dict:
            attn = attn.get('copy', None)
        #if attn is not None:
        #   print(attn.size())
        #   attn = attn[:, -1, :]

        decoder_out[1]['copy'] = attn

        probs = model.get_normalized_probs(decoder_out, log_probs=log_probs, sample=sample)
        #print(probs.size())
        #print(probs.view(-1, 1, probs.size(1)).size())
        #print(probs.view(-1, 1, probs.size(1))[:, -1, :].size())
        #probs = probs[:, -1, :]
