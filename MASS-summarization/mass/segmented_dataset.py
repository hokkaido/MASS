...
# sequence = B x T
def create_segments(sequence, segment_tokens):
    ...
    return torch.cat((append, narrowed), 1)
...
class SegmentedLanguagePairDataset(BaseWrapperDataset):
    ...
    """
    def __init__(self, dataset, embed_entities=False, segment_tokens_idx=None):
    ...
    def collater(self, samples):
        ...
        if self.segment_tokens_idx is not None:
            ...
            batch['net_input']['src_segment_labels'] = create_segments(src_item, self.segment_tokens_idx)
            batch['net_input']['tgt_segment_labels'] = create_segments(tgt_item, self.segment_tokens_idx)
        return batch
